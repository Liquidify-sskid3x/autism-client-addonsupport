...
public final class AutismPayloadTemplate {
    ...
    private static void writeField(ByteArrayOutputStream out, Field field, EncodingMode mode, List<String> warnings) throws Exception {
        ...
        switch (field.type()) {
            ...
            case JSON_STRING, TEXT_COMPONENT, NBT, ITEM_STACK -> out.write(value.getBytes(StandardCharsets.UTF_8));
            case JAVA_WRITE_UTF -> {
        ...
        if ((field.type() == FieldType.NBT || field.type() == FieldType.ITEM_STACK) && warnings != null) {
            warnings.add(field.type().label() + " is sent as text/SNBT unless a packet-specific schema is known.");
        }
    }
