...
public class UseItemPhaseAction implements MacroAction {
    ...
    public void execute(Minecraft mc) {
        ...
        InteractionHand h = "OFF_HAND".equalsIgnoreCase(hand) ? InteractionHand.OFF_HAND : InteractionHand.MAIN_HAND;
        int times = Math.max(1, Math.min(1000, repeat));
        for (int i = 0; i < times; i++) {
            switch (phase) {
                case USE_ONCE, START_USE -> mc.getConnection().send(new ServerboundUseItemPacket(h, 0, mc.player.getYRot(), mc.player.getXRot()));
                ...
                case USE_BLOCK -> {
                    if (mc.hitResult instanceof BlockHitResult bhr && mc.hitResult.getType() == HitResult.Type.BLOCK) {
                        ...
                        mc.getConnection().send(new ServerboundUseItemPacket(h, 0, mc.player.getYRot(), mc.player.getXRot()));
                    }
                }
                case SWING -> mc.getConnection().send(new ServerboundSwingPacket(h));
            }
            boolean gateInstalled = false;
            if (gateDuringHold && holdTicks > 0) {
            ...
            try {
                ...
                if (gateInstalled) PacketGateManager.disable(gateId);
            }
    ...
    private void clickSlot(Minecraft mc, int slot, int button, ContainerInput input) {
        if (mc.player == null || mc.player.containerMenu == null || mc.getConnection() == null) return;
